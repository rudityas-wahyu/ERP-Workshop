...
export default function AddInventoryModal({ isOpen, onClose, onSuccess }: { isOpen: boolean; onClose: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('inventory').insert([{ 
      sku, barcode, product_name: productName, category, subcategory, brand, compatible, cost_price: costPrice, selling_price: sellingPrice, stock, location_shelf: locationShelf, supplier, created_at: new Date(createdAt).toISOString() 
    }]);
    ...
    if (!error) {
      await supabase.from('audit_log').insert([{
        ...
        event: 'Stock In',
        details: `${stock}x ${productName} added`
      }]);
    }
    
    setLoading(false);
    if (!error) {
      setSku(''); setBarcode(''); setProductName(''); setCategory(''); setSubcategory(''); setBrand(''); setCompatible(''); setCostPrice(0); setSellingPrice(0); setStock(0); setLocationShelf(''); setSupplier('');
      onSuccess();
      onClose();
    } else {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm overflow-y-auto pt-24 pb-12">
      <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-6 w-full max-w-3xl shadow-2xl mt-auto mb-auto">
        <h2 className="text-lg font-medium text-zinc-100 mb-6">Add Inventory Item</h2>
