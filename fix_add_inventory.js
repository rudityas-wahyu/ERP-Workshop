...
let content = fs.readFileSync('src/components/modals/AddInventoryModal.tsx', 'utf-8');
...
if (!content.includes('createdAt')) {
  ...
  content = content.replace(
    "location_shelf: locationShelf, supplier",
    "location_shelf: locationShelf, supplier, created_at: new Date(createdAt).toISOString()"
  );
...
fs.writeFileSync('src/components/modals/AddInventoryModal.tsx', content);
